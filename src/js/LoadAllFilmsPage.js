...
export default class LoadAllFilmsPage extends EventTarget {
  ...
  async start(listContainer) {
    try {
      ...
      console.log('Full API Response:', filmsFromApi); 
  ...
  renderFilm(data) {
    ...
    movieCard.innerHTML = `
        ...
        <p class="moviesSecond__list__elem__desc">
            ${movieData.intro || 'No description available'} 
            <strong>${movieData.year ? `(${movieData.year})` : ''}</strong>
        </p>
        ...
        <a href="movie.html?id=${data.id}" class="moviesSecond__list__elem__link">More Info</a>
    `;
